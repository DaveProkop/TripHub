...
<template>
  <div class="card hover:shadow-soft transition-shadow duration-200 cursor-pointer">
    <!-- Photos preview -->
    <div v-if="trip.photos && trip.photos.length > 0" class="mb-3 -mx-4 -mt-4 rounded-t-2xl overflow-hidden h-40">
      <img
        :src="trip.photos[0]"
        :alt="trip.name"
    ...
    <div class="flex items-start justify-between gap-2">
      <router-link :to="`/trips/${trip.id}`" class="flex-1 min-w-0">
        ...
        <p v-if="trip.description" class="text-gray-500 dark:text-gray-400 text-sm mt-1 line-clamp-2">
          {{ trip.description }}
        </p>
        <!-- GPS -->
        <div v-if="trip.lat && trip.lng" class="flex items-center gap-1 mt-2 text-xs text-gray-400 dark:text-gray-500">
    ...
    <!-- Photo count if more than 1 -->
    <div v-if="trip.photos && trip.photos.length > 1" class="mt-2 text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1">
      <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
      <span>{{ trip.photos.length }} fotek</span>
    </div>
  </div>
</template>
